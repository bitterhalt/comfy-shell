...
class BluetoothDeviceItem(widgets.Button):
    """Individual Bluetooth device button with native connect/disconnect"""
...
class BluetoothSection(widgets.Box):
    """Bluetooth section with pill and expandable device list"""
    ...
    def _get_status_text(self) -> str:
        """Get status text (battery or device count)"""
        try:
            if not bluetooth.powered:
                return ""
    ...
    def _on_devices_changed(self):
        """Handle connected devices list changing"""
        self._device_signals.disconnect_all()
    ...
    def _toggle_bluetooth(self):
        """Toggle Bluetooth on/off using native API"""
        try:
    ...
    def _open_bluetooth_manager(self):
        """Open Bluetooth manager from config"""
